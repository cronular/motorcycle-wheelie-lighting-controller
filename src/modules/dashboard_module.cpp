...
<style>
.wizardOverlay{position:fixed;z-index:20;inset:0;display:none;place-items:center;padding:16px;background:rgba(3,6,10,.88);backdrop-filter:blur(8px)}.wizardOverlay.open{display:grid}.wizardPanel{width:min(520px,100%);padding:24px;border:1px solid #38506c;border-radius:22px;background:linear-gradient(180deg,#172231,#0d141e);box-shadow:0 28px 90px #000}.wizardStep{color:var(--accent);font-size:11px;font-weight:850;letter-spacing:.14em;text-transform:uppercase}.wizardPanel h2{margin:9px 0;font-size:clamp(25px,7vw,36px)}.wizardPanel p{min-height:54px;color:var(--muted);line-height:1.55}.wizardGraphic{height:120px;display:grid;place-items:center;margin:18px 0;border:1px solid var(--line);border-radius:15px;background:#080d14;font-size:55px}.wizardProgress{height:8px;overflow:hidden;border-radius:99px;background:#080d14}.wizardProgress i{display:block;width:0;height:100%;background:linear-gradient(90deg,var(--accent),var(--accent2));transition:width .2s linear}.wizardTime{margin-top:7px;text-align:right;color:var(--muted);font-size:11px}.wizardPanel .actions{margin-top:18px}@media(max-width:460px){.wizardPanel{padding:19px}}
...
</div><div class="message" id="message">Ready</div></div><div id="wizardOverlay" class="wizardOverlay" role="dialog" aria-modal="true" aria-labelledby="wizardTitle"><div class="wizardPanel"><div id="wizardStep" class="wizardStep">Before you begin</div><h2 id="wizardTitle">Park securely</h2><div id="wizardGraphic" class="wizardGraphic" aria-hidden="true">🏍️</div><p id="wizardInstruction">Use level ground, keep both wheels down, and stand beside the motorcycle. The light output will be locked off.</p><div class="wizardProgress"><i id="wizardProgress"></i></div><div id="wizardTime" class="wizardTime">About 20 seconds</div><div class="actions"><button id="wizardCancel" onclick="closeOrientationWizard()">Not now</button><button id="wizardStart" class="primary" onclick="startOrientationWizard()">Start setup</button></div></div></div>
<script>
...
let wizardRunning=false,wizardTimer=0,wizardWakeLock=null;
function wizardScreen(step,title,instruction,graphic,progress,time){$('wizardStep').textContent=step;$('wizardTitle').textContent=title;$('wizardInstruction').textContent=instruction;$('wizardGraphic').textContent=graphic;$('wizardProgress').style.width=progress+'%';$('wizardTime').textContent=time}
function openOrientationWizard(){if(wizardRunning)return;$('wizardOverlay').classList.add('open');wizardScreen('Before you begin','Park securely','Use level ground, keep both wheels down, and stand beside the motorcycle. The light output will be locked off.','🏍️',0,'About 20 seconds')}
function closeOrientationWizard(){if(!wizardRunning)$('wizardOverlay').classList.remove('open')}
async function startOrientationWizard(){if(wizardRunning||!token)return;wizardRunning=true;$('wizardStart').disabled=true;$('wizardCancel').classList.add('hidden');msg('Mounting setup in progress');try{if(navigator.wakeLock)wizardWakeLock=await navigator.wakeLock.request('screen')}catch(e){}const started=performance.now();const update=()=>{const s=(performance.now()-started)/1000;if(s<4)wizardScreen('Step 1 of 3','Hold the bike upright','Keep the motorcycle still and vertical. Reading begins when the countdown ends.','↕️',Math.min(20,s/4*20),'Reading in '+Math.max(1,Math.ceil(4-s))+' sec');else if(s<5.5)wizardScreen('Step 1 of 3','Keep holding still','The controller is measuring the upright reference.','⏸️',20+(s-4)/1.5*15,'Hold steady');else if(s<13.5)wizardScreen('Step 2 of 3','Lean side to side','Smoothly lean the parked motorcycle left and right several times. Keep both wheels on the ground.','↔️',35+(s-5.5)/8*45,Math.max(1,Math.ceil(13.5-s))+' sec remaining');else wizardScreen('Step 3 of 3','Return upright','Hold the bike still and upright while the controller calibrates level.','↕️',Math.min(97,80+(s-13.5)/10*17),'Calibrating…')};update();wizardTimer=setInterval(update,200);try{const r=await fetch('/api/orientation?token='+encodeURIComponent(token),{method:'POST'});const result=await r.text();clearInterval(wizardTimer);const saved=r.ok||result.startsWith('Orientation saved');wizardScreen(r.ok?'Setup complete':'Setup needs another try',saved?'Mounting saved':'Could not detect mounting',result,r.ok?'✓':'!',100,r.ok?'Ready to ride':'Review your position and retry');msg(result);if(r.ok){settingsLoaded=false;axisLoaded=false;$('wizardStart').textContent='Done';$('wizardStart').onclick=()=>location.reload()}else{$('wizardStart').textContent='Try again';$('wizardStart').onclick=()=>{wizardRunning=false;$('wizardStart').disabled=false;$('wizardCancel').classList.remove('hidden');startOrientationWizard()}}}catch(e){clearInterval(wizardTimer);wizardScreen('Connection interrupted','Check setup status','The controller may still be finishing setup. Reconnect to its Wi-Fi, then reload this page.','!',100,'Connection lost');msg('Wizard connection interrupted');$('wizardStart').textContent='Reload';$('wizardStart').onclick=()=>location.reload()}finally{if(wizardWakeLock)try{await wizardWakeLock.release()}catch(e){}wizardWakeLock=null;$('wizardStart').disabled=false}}
